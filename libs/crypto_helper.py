...
def get_user_hashes(key_hash, entrants_list):
    'Generate a list of tuples with the user key'
...
def get_winning_order(key_hash, dice_hash, entrants_list):
    'Generate a sorted list of the entrants, using lowest value of xor of their digest and the roll digest'
...
def get_nist_hash(timestamp):
    ...
    deb(nist_url)
    try:
        pulse = requests.get(nist_url, timeout=5).json()['pulse']['outputValue']
    except:
...
def get_bitcoin_hash(timestamp):
    ...
    try:
        ...
        blocks = requests.get('https://blockchain.info/blocks/' + str((timestamp+1000)*1000), params={'format':'json'}, timeout=5).json()
