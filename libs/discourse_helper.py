...
class discouse_connection:
    def __init__(self,url,discord_api_key) -> None:

        self.discource_client = DiscourseClient(
            url,
            api_username='system',
    ...
    def get_all_voters(self,post_id, poll_name, option_id):
        results=[]
        i=1
        page = self.discource_client._request("GET","/polls/voters.json",params={"post_id":post_id,"poll_name":poll_name,"option_id":option_id, "page":i})['voters'][option_id] # Hacky way to get voters directly
        results+=page
        i+=1
        while len(page) !=0:
            page = self.discource_client._request("GET","/polls/voters.json",params={"post_id":post_id,"poll_name":poll_name,"option_id":option_id, "page":i})['voters'][option_id]
            results+=page
            i+=1
    ...
    def get_all_polls(self,post_id,close_time_override=None):
        ...
        topic=self.discource_client.topic_posts(str(post_id))
        all_poll_items=[]
                    ...
                    for item in poll['options']:
                        ...
                        if close_time_override:
                            ...
                            try:
                                ...
                                exit()
                        winnable_item['voters'] = self.get_all_voters(post['id'],poll['name'],item['id'])
