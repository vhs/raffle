...
def main():
    parser = argparse.ArgumentParser(
        description="Run raffels for VHS. Randomness from Bitcoin and NIST"
    )
    parser.add_argument(
        "post_id",
    ...
    parser.add_argument(
        "--api_key_file", action="store", help="Read API key from a file"
    )
    parser.add_argument(
        "--url",
        action="store",
    ...
    for item in discouse_connection.get_all_polls(args.post_id):
        ...
        if time.time() > item["close_time"]:
            ...
            dice_hash = libs.crypto_helper.get_hash(
                bitcoin_hash.hex() + nist_hash.hex()
            )
            # print(f"Dice hash (hash(Bitcoin + NIST)): {dice_hash.hex()}")
            # print(get_hash(key_hash.hex()+" "+dice_hash.hex()).hex())
            winning = libs.crypto_helper.get_winning_order(
                key_hash, dice_hash, item["voters"]
            )
